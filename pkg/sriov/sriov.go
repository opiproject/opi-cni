...
type Manager interface {
	SetupVF(conf *opitypes.NetConf, podifName string, netns ns.NetNS) (string, error)
	ReleaseVF(conf *opitypes.NetConf, netns ns.NetNS, netNSPath string) error
	ResetVFConfig(conf *opitypes.NetConf) error
	ResetVF(conf *opitypes.NetConf) error
	ApplyVFConfig(conf *opitypes.NetConf) error
	FillOriginalVfInfo(conf *opitypes.NetConf) error
}
...
func (s *sriovManager) SetupVF(conf *opitypes.NetConf, podifName string, netns ns.NetNS) (string, error) {
...
func (s *sriovManager) ReleaseVF(conf *opitypes.NetConf, netns ns.NetNS, netNSPath string) error {
...
func (s *sriovManager) ApplyVFConfig(conf *opitypes.NetConf) error {
	if conf.MAC != "" {
...
/*func (s *sriovManager) FillOriginalVfInfo(conf *opitypes.NetConf) error {
	pfLink, err := s.nLink.LinkByName(conf.Master)
	if err != nil {
		return fmt.Errorf("failed to lookup master %q: %v", conf.Master, err)
	}
...
// FillOriginalVfInfo fills the original vf info
func (s *sriovManager) FillOriginalVfInfo(_ *opitypes.NetConf) error {
	return nil
}

// ResetVFConfig reset a VF to its original administrative state
func (s *sriovManager) ResetVFConfig(conf *opitypes.NetConf) error {
	if conf.MAC != "" {
...
func (s *sriovManager) ResetVF(conf *opitypes.NetConf) error {
